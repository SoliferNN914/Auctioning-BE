...
function seed({ userData, auctionData, businessesData }) {
  return db
    ...
    db.query(`
      CREATE TABLE auctions (
        ...
        seat_selection TEXT ARRAY[],
        current_price INT,
        time_started TIMESTAMP,
        time_ending TIMESTAMP,
        current_highest_bidder INT REFERENCES users(user_id),
        users_involved INT ARRAY[],
        active BOOLEAN DEFAULT true,
        bid_counter INT DEFAULT 1
      )`))
      .then(() => {
        const insertAuctionsQueryStr = format(
          'INSERT INTO auctions (event_id, seat_selection, current_price, time_started, time_ending, current_highest_bidder, users_involved, users_involved, active, bid_counter) VALUES %L;',
          auctionData.map(({ event_id, seat_selection, current_price, time_started, time_ending, current_highest_bidder, users_involved, users_involved, active, bid_counter }) => [event_id, seat_selection, current_price, time_started, time_ending, current_highest_bidder, users_involved, users_involved, active, bid_counter])
        )
